...
#include "../PgnParser/Chessboard.hpp"
...
TEST_CASE("MakeMove")
{
   ...
   REQUIRE(cb1[6] == Piece(PieceType::KING, Color::WHITE));
   REQUIRE(cb1[5] == Piece(PieceType::ROOK, Color::WHITE));

   cb1 = cb;
   cb1.MakeMove(4, 2, PieceType::NONE);
   REQUIRE(cb1[2] == Piece(PieceType::KING, Color::WHITE));
   REQUIRE(cb1[3] == Piece(PieceType::ROOK, Color::WHITE));

   cb1 = cb;
   cb1.MakeMove(14, 23, PieceType::NONE);
   REQUIRE(cb1[23] == Piece(PieceType::PAWN, Color::WHITE));
   REQUIRE(cb1[14] == Piece());

   cb1 = cb;
   cb1.MakeMove(12, 19, PieceType::NONE);
   REQUIRE(cb1[19] == Piece(PieceType::BISHOP, Color::WHITE));
   REQUIRE(cb1[12] == Piece());

   cb1 = cb;
   cb1.MakeMove(50, 57, PieceType::QUEEN);
   REQUIRE(cb1[57] == Piece(PieceType::QUEEN, Color::WHITE));
   REQUIRE(cb1[50] == Piece());

   cb1 = cb;
   cb1.MakeMove(39, 46, PieceType::NONE);
   REQUIRE(cb1[46] == Piece(PieceType::PAWN, Color::WHITE));
   REQUIRE(cb1[39] == Piece());
   REQUIRE(cb1[38] == Piece());
}